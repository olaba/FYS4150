...
int main(int argc, char* argv[])
{
    ...
    idum = time(NULL); // random starting point based on time this time! :)
    ...
    //for (double temp = initial_temp; temp <= final_temp; temp+=temp_step){
		...
		// update expectation values. These must be divided by number of cycles afterward?!
...
//Initialize magnetization and calculate M
    for(int y = 0; y < n_spins; y++){
        for(int x = 0; x < n_spins; x++){
                ...
                //spin orientation "ground state". If temp low enough, all spins are aligned one way (up). Comment out if you want random config #vivalebruteforce
                //spin_matrix[y][x] = 1; //comment out if you want random start config #vivelabruteforce!

                //A sad try to make a random configuration. Comment out if you want "all up"  initial config.
